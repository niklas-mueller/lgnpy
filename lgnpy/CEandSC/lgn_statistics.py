...
def conv2padded(varargin):
    ...
    for p in range(x.shape[2]):
        if len(varargin) == 2:
            ...
            ans2 = cv2.filter2D(x, -1, h, borderType=cv2.BORDER_REPLICATE)
            x[:, :, p] = ans2
        else:
...
def lgn_statistics(im, file_name:str, threshold_lgn, viewing_dist=1, dot_pitch=0.00035, fov_beta=1.5, fov_gamma=5, 
    ...
    if force_recompute or results is None:
        ...
        results = np.array((par1, par2, par2, mag1, mag2, mag3))
    ...
    # if compute_extra_statistics:
    #     # Peripherie
    #     sc_extra.append([np.mean(np.abs(mag1[~imfovgamma])), np.mean(
    #         np.abs(mag2[~imfovgamma])), np.mean(np.abs(mag3[~imfovgamma]))])

    #     # Custom boxes (crops)
    #     for mask in crop_masks:
    #         sc_extra.append([np.mean(np.abs(mag1[mask])), np.mean(
